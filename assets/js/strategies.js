...
const STRATEGY_LIBRARY = {
  ...
  triangles: {
    ...
    description: "Triangles are consolidations bounded by two converging trendlines. An ascending triangle has a flat resistance line and a rising support line — buyers stepping in at progressively higher prices against a fixed ceiling — and is generally read as bullish, expected to break upward. A descending triangle is the mirror image (flat support, falling resistance) and is generally read as bearish. A symmetrical triangle has both lines converging toward each other and is more genuinely neutral, with the eventual breakout direction, often in line with the prevailing trend, doing the real work of the signal rather than the shape itself.",
  ...
  wedges: {
    ...
    description: "A wedge looks similar to a triangle but both boundary lines slope in the same direction rather than one being flat. A rising wedge — higher highs and higher lows, but the highs rising more slowly than the lows, so the range narrows — is counterintuitively a bearish pattern, since the shrinking range shows buying momentum fading even as price grinds higher; it typically breaks down. A falling wedge is the mirror image and typically breaks up. Because the direction of the slope doesn't match the eventual breakout direction, wedges are a common source of confusion versus channels, which slope with the trend and don't imply a reversal.",
    citation: 'Edwards, Robert D., and John Magee. Technical Analysis of Stock Trends. 9th ed., CRC Press, 2007, ch. 9.',
  ...
  sector_rotation: {
    ...
    description: "Sector rotation allocates toward the sectors that have historically performed best at a given stage of the business cycle: early-cycle recovery tends to favor cyclicals, small caps, and financials as credit conditions ease; mid-cycle expansion broadens out to industrials and technology; late-cycle tends to favor energy and materials as inflation pressures build; and contraction/recession tends to favor defensives — consumer staples, utilities, healthcare — as growth slows. This ties directly into Fed policy and yield-curve signals (this desk's Fed Watch coverage is a natural input), though real cycles rarely follow the textbook sequence exactly, and calling the current stage of the cycle in real time is itself the hard part.",
  ...
  index_rebalancing: {
    ...
    description: "When a widely-tracked index adds or removes a constituent, every fund benchmarked to that index — passive trackers and closet-indexers alike — needs to buy or sell the stock to stay in line, regardless of their view on its valuation. This creates predictable, mechanical demand (or supply) around the announcement and effective dates, historically producing a measurable price effect independent of the company's fundamentals. The effect has become somewhat smaller and more front-run over time as more capital anticipates it ahead of the effective date, but it remains a distinct, well-documented catalyst worth tracking around S&P 500, Russell, and other major index reconstitution events.",
  ...
  merger_arbitrage: {
    ...
    description: "When one company agrees to acquire another, the target's stock typically jumps toward — but not fully to — the announced deal price, with the remaining gap reflecting the market's estimate of the probability the deal closes, the time until closing, and financing/regulatory risk. Merger arbitrage buys the target (and, in a stock-for-stock deal, may short the acquirer to hedge out that stock's own price risk) to capture that spread as it narrows toward zero when the deal completes. The return profile is asymmetric: modest, relatively steady gains if the deal closes as expected, against a large loss if the deal breaks (regulatory blocks, financing falling through, shareholder rejection) and the target's stock falls back toward its pre-announcement level.",
